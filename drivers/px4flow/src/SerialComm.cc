...
namespace px {
    ...
    SerialComm::SerialComm(const std::string &frameId)
            : m_port(m_uartService), m_timer(m_uartService), m_imageSize(0), m_imagePackets(0), m_imagePayload(0),
              m_imageWidth(0), m_imageHeight(0), m_systemId(-1), m_compId(0), m_frameId(frameId), m_timeout(false),
              m_errorCount(0), m_connected(false) {
    ...
    SerialComm::open(const std::string &portStr, int baudrate) {
        ...
        m_twistPub = nh.advertise<geometry_msgs::TwistWithCovarianceStamped>("px4_flow", 5);
            ...
            if (msgReceived) {
                ...
                switch (message.msgid) {
                    case MAVLINK_MSG_ID_OPTICAL_FLOW: {
                        ...
                        optFlowMsg.header.stamp = ros::Time(flow.time_usec / 1000000,
                                                            (flow.time_usec % 1000000) * 1000);
                        ...
                        twistMsg.twist.twist.linear.x = flow.flow_comp_m_x;
                        twistMsg.twist.twist.linear.y = flow.flow_comp_m_y;
                        twistMsg.twist.twist.angular.z = 0.0;
